...
# Listings API with status and security filter
...
<head>
  ...
  <style> body{{font-family:Inter,system-ui,Arial;margin:20px;}} a{{color:#2563eb}} .tag{{display:inline-block;padding:4px 8px;border-radius:999px;font-size:12px}} .danger{{background:#fee2e2;color:#991b1b}} .warn{{background:#fef3c7;color:#92400e}} .safe{{background:#dcfce7;color:#166534}} </style>
</head>
<body>
  <h1 style="margin:0 0 8px 0;">{title}</h1>
  <div style="color:#475569;margin-bottom:10px;">£{price} • {profile} • score {score:.2f}</div>
  <div style="margin-bottom:10px;">Security: <span class="tag {tagclass}">{tagtext} — {sec}/100</span></div>
...
def _tag_for(sec:int):
    if sec >= 96: return "safe", "Safe"
    if sec >= 86: return "warn", "Low risk"
    return "danger", "Scam alert"
...
def item_page(item_id: int):
    ...
    tagclass, tagtext = _tag_for(int(row.get("security_score") or 0))
    html = DETAIL_TEMPLATE.format(
        ...
        tagclass=tagclass, tagtext=tagtext
    )
